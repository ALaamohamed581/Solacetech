export const personalDataSections = [
  {
    title: "Basic Information",
    rows: [
      [
        {
          name: "National ID Number",
          value: "29702031400693",
          inputName: "nationalId",
        },
        {
          name: "National ID Expiring Date",
          value: "01 / 04 / 2025",
          inputName: "nationalId",
        },
        { name: "Title", value: "Mr." },
      ],
      [
        { name: "First Name", value: "John", inputName: "firstName" },
        { name: "Father Name", value: "John", inputName: "fatherName" },
        {
          name: "Grand Father Name",
          value: "John",
          inputName: "grandfatherName",
        },
        { name: "Family Name", value: "Smith", inputName: "familyName" },
      ],
      [
        { name: "الأسم الأول", value: "جون", inputName: "localizedName" },
        { name: "اسم الأب", value: "جون" },
        { name: "اسم الجد", value: "جون" },
        { name: "اللقب / اسم العائلة", value: "سميث" },
      ],
      [
        { name: "Date of Birth", value: "01 / 04 / 1980" },
        { name: "Gender", value: "Male" },
        { name: "Nationality", inputName: "nationalities", value: "Egyptian" },
        { name: "Additional Nationality", value: "-" },
      ],
      [
        { name: "Passport No.", value: "A135464" },
        { name: "Passport Issue Date", value: "01 / 04 / 1980" },
        { name: "Passport Expiry Date", value: "01 / 04 / 1980" },
      ],
      [
        { name: "Marital Status", value: "Single" },
        { name: "Dependencies", value: "0", inputName: "dependants" },
      ],
      [{ name: "Status", value: "Active" }],
    ],
  },
  {
    title: "Contact Information",
    rows: [
      [
        { name: "Personal Email", value: "John.smith@gmail.com " },
        { name: "Mobile", value: "011223344556" },
      ],
    ],
  },
  {
    title: "Emergency Contacts",
    rows: [
      [
        { name: "Emergency Contact Person Name", value: "John John" },
        { name: "Emergency Relation", value: "Father" },
        { name: "Emergency Phone", value: "011224477885" },
      ],
    ],
  },
  {
    title: "Address Details",
    rows: [
      [
        { name: "Country", value: "Egypt" },
        { name: "City", value: "Cairo" },
        { name: "Postal Code", value: "11728" },
      ],
      [
        { name: "Building", value: "7" },
        { name: "Street", value: "7" },
        { name: "Floor No.", value: "72" },
        { name: "Apartment", value: "7" },
      ],
    ],
  },
  {
    title: "Driving License Details",
    rows: [
      [
        { name: "Driving License", value: "Yes" },
        { name: "Driving License Type", value: "C1E" },
        { name: "Driving License expiry date", value: "01 / 04 / 2025" },
      ],
    ],
  },
  {
    title: "Military Status",
    rows: [
      [
        { name: "Require Travel Permit", value: "Yes" },
        { name: "Military Status", value: "Exempted " },
        { name: "Document", value: "filename1.docx" },
      ],
    ],
  },
];
